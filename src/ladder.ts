/**
 * GANSU Lite — Post-HF Method Ladder demo page.
 * Compares RHF → MP2 → MP3 → CCSD energies for the same molecule,
 * visualising correlation energy recovery as a staircase bar chart.
 */
...
const METHODS = ['RHF', 'MP2', 'MP3', 'CCSD'] as const;
type Method = typeof METHODS[number];

interface LadderResult {
  method: Method;
  totalEnergy: number;
  corrEnergy: number;   // correlation energy (0 for RHF)
  recoveryPct: number;  // % of CCSD correlation recovered
  timeMs: number;
  error?: string;
}
...
const METHOD_COLORS_LIGHT: Record<Method, string> = {
  RHF: '#1f77b4',
  MP2: '#2ca02c',
  MP3: '#ff7f0e',
  CCSD: '#d62728',
};
const METHOD_COLORS_DARK: Record<Method, string> = {
  RHF: '#4dabf7',
  MP2: '#51cf66',
  MP3: '#ffa94d',
  CCSD: '#ff6b6b',
...
function methodColor(m: Method): string {
  return isDark() ? METHOD_COLORS_DARK[m] : METHOD_COLORS_LIGHT[m];
}
    ...
    <div class="opt-page">
      ...
      <div class="opt-content">
        <div class="opt-panel opt-controls">
          <h2>${t('ladder.molecule')}</h2>
          <div class="opt-scenario-grid" id="scen-grid"></div>

          <div class="ladder-basis-info">
            <span style="color:var(--color-text-dim);font-size:0.72rem">${t('ladder.basis')}: STO-3G</span>
          </div>

          <button id="run-btn" class="opt-run-btn" ${running ? 'disabled' : ''}>
            ${running ? t('ladder.running') : t('ladder.run')}
          </button>
          ${running ? `<button id="stop-btn" class="opt-stop-btn">${t('ladder.stop')}</button>` : ''}
        ...
        <div class="opt-panel opt-graph-panel">
          <div id="graph-container">
            ${!done && !running
              ? `<p class="opt-hint">${t('ladder.waiting')}</p>`
              : ''}
          </div>
        </div>
      </div>
...
async function runLadder(): Promise<void> {
  ...
  try {
    ...
    const eHF = await rhf.solve({ eriBackend: 'js' });
    const rhfTime = performance.now() - t0;
    ...
    const eCorrMP2 = computeMP2Energy(C, eps, eri, nocc, nbasis);
    const mp2Time = performance.now() - t1;

    results.push({
      method: 'MP2',
      totalEnergy: eHF + eCorrMP2,
      corrEnergy: eCorrMP2,
      recoveryPct: 0, // updated after CCSD
      timeMs: mp2Time,
    });
    renderGraph();
    await new Promise<void>(r => setTimeout(r, 0));
    ...
    const totalCorrMP3 = mp3Result.mp2 + mp3Result.mp3;

    results.push({
      method: 'MP3',
      totalEnergy: eHF + totalCorrMP3,
      corrEnergy: totalCorrMP3,
      recoveryPct: 0, // updated after CCSD
      timeMs: mp3Time,
    });
    renderGraph();
    ...
    const eCorrCCSD = computeCCSDEnergy(C, eps, eri, nocc, nbasis);
    const ccsdTime = performance.now() - t3;

    results.push({
      method: 'CCSD',
      totalEnergy: eHF + eCorrCCSD,
      corrEnergy: eCorrCCSD,
      recoveryPct: 100,
      timeMs: ccsdTime,
    });

    // Update recovery percentages
    if (Math.abs(eCorrCCSD) > 1e-12) {
      for (const r of results) {
        if (r.method !== 'RHF' && r.method !== 'CCSD') {
          r.recoveryPct = (r.corrEnergy / eCorrCCSD) * 100;
        }
      }
    }

  } catch (err) {
    console.error('Ladder computation error:', err);
  }
...
function renderSummary(): void {
  ...
  let html = `<div class="opt-summary">
    <h3>${done ? t('ladder.done') : t('ladder.running')}</h3>
    <table>
      <tr>
        <th>${t('ladder.colMethod')}</th>
        <th>${t('ladder.colEnergy')}</th>
        <th>${t('ladder.colCorr')}</th>
        <th>${t('ladder.colRecovery')}</th>
        <th>${t('ladder.colTime')}</th>
      </tr>`;

  for (const r of results) {
    const eStr = r.totalEnergy.toFixed(6);
    const cStr = r.method === 'RHF' ? '\u2014' : r.corrEnergy.toFixed(6);
    const rStr = r.method === 'RHF' ? '\u2014' : `${r.recoveryPct.toFixed(1)}%`;
    const tStr = r.timeMs < 1000 ? `${r.timeMs.toFixed(0)}ms` : `${(r.timeMs / 1000).toFixed(1)}s`;
    const color = methodColor(r.method);
    html += `<tr>
      <td><span style="color:${color};font-weight:600">${r.method}</span></td>
      <td style="font-family:monospace">${eStr}</td>
      <td style="font-family:monospace">${cStr}</td>
  ...
  html += `</table>
    ${done ? `<div style="margin-top:6px;font-size:0.72rem;color:var(--color-text-dim)">${t('ladder.totalTime')}: ${(totalTime / 1000).toFixed(1)}s</div>` : ''}
  </div>`;
  el.innerHTML = html;
}
...
function renderGraph(): void {
  ...
  container.innerHTML = renderEnergyLevelChart() + renderRecoveryChart();
}
...
function renderEnergyLevelChart(): string {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Filled bar from RHF energy to this method's energy (only for post-HF)
    if (i > 0) {
      const yRHF = toY(results[0].totalEnergy);
      const barX = ml + barW * i + barW * 0.15;
      const bw = barW * 0.7;
      const barTop = Math.min(y, yRHF);
      const barH = Math.abs(yRHF - y);
      svg += `<rect x="${barX}" y="${barTop}" width="${bw}" height="${barH}" fill="${color}" opacity="0.25" rx="2"/>`;
    }
    ...
    svg += `<text x="${cx}" y="${mt + ph + 16}" text-anchor="middle" font-size="10" fill="${tc.dim}" font-weight="600">${r.method}</text>`;
  ...
  // Correlation energy bracket (RHF to CCSD)
  if (n === METHODS.length) {
    const ccsdR = results[n - 1];
    const rhfR = results[0];
    const bx = ml + pw + 4;
    const y1 = toY(rhfR.totalEnergy);
    const y2 = toY(ccsdR.totalEnergy);
    const bracketColor = isDark() ? '#ffd700' : '#cc8800';
    svg += `<line x1="${bx}" y1="${y1}" x2="${bx}" y2="${y2}" stroke="${bracketColor}" stroke-width="1.5"/>`;
    svg += `<line x1="${bx - 3}" y1="${y1}" x2="${bx + 3}" y2="${y1}" stroke="${bracketColor}" stroke-width="1.5"/>`;
    svg += `<line x1="${bx - 3}" y1="${y2}" x2="${bx + 3}" y2="${y2}" stroke="${bracketColor}" stroke-width="1.5"/>`;
  }

  // Title & Y-axis label
  svg += `<text x="${ml + pw / 2}" y="20" text-anchor="middle" font-size="12" font-weight="600" fill="${tc.titleSvg}">${t('ladder.graphTitle')}</text>`;
  svg += `<text x="14" y="${mt + ph / 2}" text-anchor="middle" font-size="10" fill="${tc.dim}" transform="rotate(-90,14,${mt + ph / 2})">${t('ladder.yEnergy')}</text>`;
  svg += `<text x="${ml + pw / 2}" y="${height - 4}" text-anchor="middle" font-size="10" fill="${tc.dim}">${t('ladder.xMethod')}</text>`;

  svg += '</svg>';
  return svg;
}

/** Chart 2: Correlation energy recovery rate bar chart */
function renderRecoveryChart(): string {
  const tc = getThemeColors();

  // Only show when we have CCSD (full results)
  const ccsdResult = results.find(r => r.method === 'CCSD');
  if (!ccsdResult || results.length < 2) return '';

  const width = 560, height = 180;
  const ml = 82, mr = 20, mt = 28, mb = 40;
  const pw = width - ml - mr, ph = height - mt - mb;

  const postHF = results.filter(r => r.method !== 'RHF');
  const n = postHF.length;
  const barW = pw / n;
  const maxPct = Math.max(110, ...postHF.map(r => r.recoveryPct));

  const toY = (pct: number) => mt + ph - (pct / maxPct) * ph;
  ...
  for (const pct of [0, 25, 50, 75, 100]) {
    const yy = toY(pct);
    svg += `<line x1="${ml}" y1="${yy}" x2="${ml + pw}" y2="${yy}" stroke="${tc.grid}" stroke-width="${pct === 100 ? 1 : 0.5}"/>`;
    svg += `<text x="${ml - 6}" y="${yy + 3}" text-anchor="end" font-size="8" fill="${tc.dim}">${pct}%</text>`;
  }

  // 100% reference line
  const y100 = toY(100);
  svg += `<line x1="${ml}" y1="${y100}" x2="${ml + pw}" y2="${y100}" stroke="${isDark() ? '#ffd700' : '#cc8800'}" stroke-width="1" stroke-dasharray="6,3" opacity="0.5"/>`;
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Percentage label
    svg += `<text x="${cx}" y="${y - 5}" text-anchor="middle" font-size="10" fill="${color}" font-weight="700">${r.recoveryPct.toFixed(1)}%</text>`;

    // Method label
    svg += `<text x="${cx}" y="${mt + ph + 14}" text-anchor="middle" font-size="10" fill="${tc.dim}" font-weight="600">${r.method}</text>`;
  }

  // Title & Y-axis
  svg += `<text x="${ml + pw / 2}" y="16" text-anchor="middle" font-size="11" font-weight="600" fill="${tc.titleSvg}">${t('ladder.recoveryTitle')}</text>`;
  svg += `<text x="14" y="${mt + ph / 2}" text-anchor="middle" font-size="9" fill="${tc.dim}" transform="rotate(-90,14,${mt + ph / 2})">${t('ladder.yRecovery')}</text>`;
  svg += `<text x="${ml + pw / 2}" y="${height - 4}" text-anchor="middle" font-size="10" fill="${tc.dim}">${t('ladder.xMethod')}</text>`;
