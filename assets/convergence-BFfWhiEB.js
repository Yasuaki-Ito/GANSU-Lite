import{i as V,c as D,r as A,t as d,a as Z,b as j,g as _,d as U}from"./nav-BKexlM7u.js";import{p as X}from"./parseXYZ-BE3rbSP_.js";import{g as Y,f as J}from"./ri-Cuj4t-H2.js";import{a as Q,H as W,b as ee}from"./theoryControls-AWp8GxPQ.js";const O=[{id:"h2",labelKey:"conv.scenH2",descKey:"conv.descH2",charge:0,mult:1,xyz:`2
H2
H  0.0  0.0  0.0
...
`)})()}],b=["STO-3G","STO-6G","3-21G","6-31G","6-31G(d,p)","cc-pVDZ","def2-SVP","def2-TZVP"],te=new Set(["cc-pVTZ","def2-TZVP"]);let k=O[0],g=[],u=!1,L=!1,$=!1,R=0,S="HF";const C=new Map;async function oe(a){const t=C.get(a);if(t)return t;const o=`/GANSU-Lite/basis/${a.toLowerCase()}.gbs`,r=await fetch(o);if(!r.ok)throw new Error(`Failed to load basis set: ${a}`);const e=await r.text(),n=J.fromGBS(e);return C.set(a,n),n}const m=document.getElementById("app");function v(){m.innerHTML=`
      ...
      <div class="opt-content">
        <div class="opt-panel opt-controls">
          ...
          <div class="theory-row" style="margin:8px 0;display:flex;align-items:center;gap:8px;font-size:0.9rem;">
            <span>Theory:</span>${Q("theory-sel",S,"",W)}
          </div>
    ...
    </div>`,se();const a=m.querySelector("#mol-grid"),t=document.createElement("div");t.className="opt-category-row";for(const r of O){const e=document.createElement("div");e.className="opt-scenario-card"+(r.id===k.id?" selected":""),e.innerHTML=`<strong>${d(r.labelKey)}</strong><span class="conv-desc">${d(r.descKey)}</span>`,e.addEventListener("click",()=>{u||(k=r,g=[],$=!1,v())}),t.appendChild(e)}a.appendChild(t),m.querySelector("#nav-theme").addEventListener("click",()=>{Z(),v()}),m.querySelector("#nav-lang").addEventListener("click",()=>{j(),v()});const o=m.querySelector("#theory-sel");o&&o.addEventListener("change",()=>{S=o.value}),m.querySelector("#run-btn").addEventListener("click",()=>{u||re()}),m.querySelector("#stop-btn")?.addEventListener("click",()=>{L=!0}),g.length>0&&(P(),G())}async function re(){u=!0,L=!1,g=[],$=!1,v();const a=Math.floor((k.mult-1)/2),t=performance.now(),o=X(k.xyz),r=S!=="HF";for(let e=0;e<b.length&&!L;e++){const n=b[e];if(r&&te.has(n)){g.push({basis:n,nbasis:0,energy:NaN,timeMs:0,error:"skipped (DFT × TZ too slow)"});continue}ne(n,e,b.length);try{const i=performance.now(),f=await oe(n),h=new Y(o,f,k.charge,a),x=await(await ee(h,f,S)).solve({eriBackend:"js"}),H=performance.now()-i;g.push({basis:n,nbasis:h.numBasis,energy:x,timeMs:H}),P(),G()}catch(i){const f=i instanceof Error?i.message:String(i);console.error(`Error computing ${n}:`,i),g.push({basis:n,nbasis:0,energy:NaN,timeMs:0,error:f})}await new Promise(i=>setTimeout(i,0))}R=performance.now()-t,u=!1,$=!0,v()}function ne(a,t,o){const r=m.querySelector("#progress-area");if(!r)return;const e=(t/o*100).toFixed(0);r.innerHTML=`
  ...
  `,document.head.appendChild(a)}V();D();v();
