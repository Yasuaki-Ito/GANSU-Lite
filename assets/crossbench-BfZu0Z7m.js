import"./styles-B8WF9G-F.js";import{p as g}from"./parseXYZ-BE3rbSP_.js";import{g as k,h as $,z as _,o as R,f as L,n as I}from"./ri-Cuj4t-H2.js";import{c as N}from"./gradient-gHuNZ-_b.js";import{c as j}from"./hessian-Co-zz1lw.js";import"./properties-DST8hwXs.js";const y=4,A=1,v=1.8897259886,O=`3
H2O standard
...
O   0.000000   0.000000  -1.160000`,x=new Map;async function w(t){if(x.has(t))return x.get(t);const a=`/GANSU-Lite/basis/${t}.gbs`,n=await fetch(a);if(!n.ok)throw new Error(`Failed to load basis ${t}: ${n.status}`);const e=L.fromGBS(await n.text());return x.set(t,e),e}async function T(t){const a=await w("6-31g(d,p)"),n=g(O),e=new k(n,a,0);await $(e,"RHF").solve({eriBackend:t})}async function D(t){const a=await w("sto-3g"),n=g(z);let e=new Float64Array(n.length*3);for(let r=0;r<n.length;r++)e[3*r]=n[r].coordinate.x,e[3*r+1]=n[r].coordinate.y,e[3*r+2]=n[r].coordinate.z;const o=.4,c=25;for(let r=0;r<c;r++){const i=n.map((s,p)=>({...s,coordinate:{x:e[3*p],y:e[3*p+1],z:e[3*p+2]}})),d=new k(i,a,0),l=$(d,"RHF");await l.solve({eriBackend:t});const m=N(d.primitiveShells,d.atoms,d.cgtoNormalizationFactors,d.numBasis,d.numAlphaSpins,l.density,l.coefficients,l.orbitalEnergies).total;let u=0;for(let s=0;s<m.length;s++)Math.abs(m[s])>u&&(u=Math.abs(m[s]));if(u<5e-4)break;for(let s=0;s<m.length;s++)e[s]-=o*m[s]}}async function U(t){const a=await w("sto-3g"),n=g(G),e=new Float64Array(n.length*3);for(let o=0;o<n.length;o++)e[3*o]=n[o].coordinate.x*v,e[3*o+1]=n[o].coordinate.y*v,e[3*o+2]=n[o].coordinate.z*v;await j(n.map(o=>o.atomicNumber),e,a,0,5e-4,void 0,void 0)}async function W(t){const a=await w("6-31g(d,p)"),n=g(O),e=new k(n,a,0);await $(e,"RHF",{functional:"B3LYP",gridLevel:"medium"}).solve({eriBackend:t})}const M=[{id:"h2o_hf_631gdp",label:"H2O HF/6-31G(d,p)",run:T},{id:"h2o_geomopt_sto3g",label:"H2O geom opt HF/STO-3G",run:D},{id:"co2_hess_sto3g",label:"CO2 Hessian HF/STO-3G",run:U},{id:"h2o_b3lyp_631gdp",label:"H2O B3LYP/6-31G(d,p)",run:W}];function J(t){const a=[...t].sort((e,o)=>e-o),n=a.length;return n===0?NaN:n%2===1?a[(n-1)/2]:(a[n/2-1]+a[n/2])/2}function F(){const t=navigator;return{ua:t.userAgent,platform:t.platform??"unknown",deviceMemoryGB:t.deviceMemory??null,hardwareConcurrency:t.hardwareConcurrency??0,screen:`${screen.width}×${screen.height}`,language:t.language||"unknown",timestamp:new Date().toISOString()}}function C(){try{const t=performance.getEntriesByType("navigation")[0];return t?Math.round(t.loadEventEnd-t.fetchStart):null}catch{return null}}async function E(){return await _("/GANSU-Lite/"),R()?I()==="wasm-simd"?{backend:"wasm",label:"WASM-SIMD"}:{backend:"wasm",label:"WASM"}:{backend:"js",label:"JS"}}const Y=document.getElementById("app");let b=null;function P(){const t=F(),a=C();Y.innerHTML=`
    <div class="cb-page">
