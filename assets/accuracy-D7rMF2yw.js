...
F2
F  0.0  0.0  0.0
F  0.0  0.0  1.412000`}],fe=["STO-3G","STO-6G","3-21G","6-31G","6-31G(d,p)","cc-pVDZ","cc-pVTZ"],F=[{id:"HF",label:"HF",category:"Post-HF",type:"hf"},{id:"MP2",label:"MP2",category:"Post-HF",type:"postHF"},{id:"MP3",label:"MP3",category:"Post-HF",type:"postHF"},{id:"CCSD",label:"CCSD",category:"Post-HF",type:"postHF"},{id:"SVWN",label:"SVWN",category:"LDA",type:"dft",functional:"SVWN"},{id:"BLYP",label:"BLYP",category:"GGA",type:"dft",functional:"BLYP"},{id:"PBE",label:"PBE",category:"GGA",type:"dft",functional:"PBE"},{id:"B3LYP",label:"B3LYP",category:"Hybrid",type:"dft",functional:"B3LYP"}];let W=te[0],I="STO-3G",T=new Set(F.map(e=>e.id)),$=[],C=!1,K=!1,G=[],O=[],V=[],Y=0,q=null,_=0,D="none";const J=new Map;async function ue(e){const t=J.get(e);if(t)return t;const o=`/Program%20Files/Git/GANSU-Lite/basis/${e.toLowerCase()}.gbs`,r=await fetch(o);if(!r.ok)throw new Error(`Failed to load basis set: ${e}`);const n=await r.text();return J.set(e,n),n}const ye={HF:"#1f77b4",MP2:"#2ca02c",MP3:"#ff7f0e",CCSD:"#d62728",SVWN:"#9467bd",BLYP:"#8c564b",PBE:"#e377c2",B3LYP:"#17becf"},$e={HF:"#4dabf7",MP2:"#51cf66",MP3:"#ffa94d",CCSD:"#ff6b6b",SVWN:"#b197fc",BLYP:"#c4956a",PBE:"#f59fda",B3LYP:"#66d9e8"};function R(e){return me()?$e[e]??"#ccc":ye[e]??"#666"}function be(){re(),G=F.map(o=>T.has(o.id)?"pending":"skipped"),O=F.map(()=>0),V=F.map(o=>T.has(o.id)?"":"skipped"),_=performance.now(),Y=0,D="none";const e=document.createElement("div");e.id="acc-progress-overlay",e.className="acc-prog-overlay";const t=document.createElement("div");t.className="acc-prog-modal",t.id="acc-progress-modal",e.appendChild(t),document.body.appendChild(e),q=setInterval(()=>{Y=performance.now()-_;const o=document.getElementById("acc-prog-elapsed");o&&(o.textContent=(Y/1e3).toFixed(1)+"s")},200),j()}function j(){const e=document.getElementById("acc-progress-modal");if(!e)return;const o=(G.filter(s=>s==="done"||s==="skipped").length/F.length*100).toFixed(0),r=(Y/1e3).toFixed(1),n=D==="wasm-simd"?"active":"inactive",d=D==="wasm"?"active":"inactive",c=D==="js"?"active":"inactive";let m=`
    ...
    <div class="acc-prog-list">`;for(let s=0;s<F.length;s++){const a=F[s],i=G[s],f=R(a.id);let y,v,w;if(i==="done"){const H=O[s]<1e3?`${O[s].toFixed(0)}ms`:`${(O[s]/1e3).toFixed(1)}s`;y='<span class="acc-prog-check">&#10003;</span>',v=H,w="acc-prog-done"}else i==="running"?(y='<span class="acc-prog-spinner"></span>',v=V[s]||"...",w="acc-prog-active"):i==="skipped"?(y='<span class="acc-prog-skip">&#8212;</span>',v="stopped",w="acc-prog-skipped"):(y='<span class="acc-prog-pending">&#9675;</span>',v="",w="acc-prog-pending-row");s===4&&(m+='<div class="acc-prog-sep"></div>'),m+=`
    ...
    <button id="acc-prog-cancel" class="acc-prog-cancel-btn">${x("acc.stop")}</button>`,e.innerHTML=m,document.getElementById("acc-prog-cancel")?.addEventListener("click",()=>{N&&(N.terminate(),N=null),Fe(0),ae()})}function oe(e,t,o){G[e]=t,o!==void 0&&(V[e]=o),j()}function ve(e,t){O[e]=t}function we(e,t){let o=t;t.includes("Converged")?o=t:t.includes("ERIs")?o="ERI "+(t.match(/(\d+)%/)?.[0]??""):t.includes("nuclear")?o="V_nn":t.includes("one-electron")?o="1e integrals":t.includes("two-electron")?o="ERI":t.includes("transformation")?o="S^{-1/2}":t.includes("SAD")?o="SAD guess":t.includes("GWH")?o="GWH guess":t.includes("initial Fock")?o="H_core guess":t.includes("grid")?o="grid switch":t.includes("SCF did not")?o="not converged":t.includes("SCF iteration")&&(o=t.replace(/^.*?(SCF iteration)/,"$1")),V[e]=o,j()}function re(){q&&(clearInterval(q),q=null),document.getElementById("acc-progress-overlay")?.remove()}const E=document.getElementById("app");function P(){const e=W.id==="h2";!e&&I==="cc-pVTZ"&&(I="cc-pVDZ");const t=fe.map(a=>{const i=a==="cc-pVTZ"&&!e?"disabled":"";return`<option value="${a}" ${a===I?"selected":""} ${i}>${a}${i?" (H₂ only)":""}</option>`}).join("");E.innerHTML=`
    <div class="opt-page">
      ${ge("accuracy")}
    ...
    </div>`,Ce();const o=E.querySelector("#scen-grid"),r=document.createElement("div");r.className="opt-category-row";for(const a of te){const i=document.createElement("div");i.className="opt-scenario-card"+(a.id===W.id?" selected":""),i.innerHTML=`<strong>${x(a.labelKey)}</strong><span class="conv-desc">${x(a.descKey)}</span>`,i.addEventListener("click",()=>{C||(W=a,$=[],K=!1,P())}),r.appendChild(i)}o.appendChild(r),E.querySelector("#basis-select").addEventListener("change",a=>{I=a.target.value,P()});const n=E.querySelector("#method-toggles"),d=document.createElement("div");d.className="acc-method-group",d.innerHTML='<span class="acc-method-group-label">Post-HF</span>';const c=document.createElement("div");c.className="acc-method-group-btns",d.appendChild(c);const m=document.createElement("div");m.className="acc-method-group",m.innerHTML='<span class="acc-method-group-label">DFT</span>';const s=document.createElement("div");s.className="acc-method-group-btns",m.appendChild(s);for(const a of F){const i=document.createElement("button"),f=a.type==="hf",y=T.has(a.id);i.className="acc-method-btn"+(y?" active":"")+(f?" always-on":""),i.textContent=a.label,i.disabled=C||f,i.style.color=y?R(a.id):"",i.title=f?"HF is always required":"",i.addEventListener("click",()=>{C||f||(T.has(a.id)?T.delete(a.id):T.add(a.id),P())}),a.type==="hf"||a.type==="postHF"?c.appendChild(i):s.appendChild(i)}n.appendChild(d),n.appendChild(m),E.querySelector("#nav-theme").addEventListener("click",()=>{he(),P()}),E.querySelector("#nav-lang").addEventListener("click",()=>{xe(),P()}),E.querySelector("#run-btn").addEventListener("click",()=>{C||ke()}),$.length>0&&(ne(),ce())}let N=null;async function ke(){C=!0,$=[],K=!1,P(),be();const e=W;try{const t=await ue(I),o=new Worker(new URL("/Program%20Files/Git/GANSU-Lite/assets/accuracyWorker-DECf1emp.js",import.meta.url),{type:"module"});N=o,o.postMessage({type:"run",xyzText:e.xyz,basisGBS:t,charge:e.charge,multiplicity:e.mult,selectedMethods:Array.from(T),baseUrl:"/Program%20Files/Git/GANSU-Lite/"}),await new Promise((r,n)=>{o.onmessage=d=>{const c=d.data;switch(c.type){case"method-state":oe(c.index,c.state,c.info);break;case"method-info":we(c.index,c.msg);break;case"method-time":ve(c.index,c.ms);break;case"backend":D=c.backend,j();break;case"method-result":$.push({method:F[c.methodIndex],totalEnergy:c.totalEnergy,diffFromHF:c.diffFromHF,timeMs:c.timeMs}),ne(),ce();break;case"done":o.terminate(),N=null,r();break;case"error":o.terminate(),N=null,n(new Error(c.message));break}},o.onerror=d=>{o.terminate(),N=null,n(new Error(d.message||"Worker failed"))}})}catch(t){t instanceof Error&&t.message==="Cancelled"||console.error("Accuracy computation error:",t)}ae()}function Fe(e){for(let t=e;t<F.length;t++)(G[t]==="pending"||G[t]==="running")&&oe(t,"skipped")}function ae(){C=!1,K=!0,setTimeout(()=>{re(),P()},600)}function ce(){const e=E.querySelector("#summary-area");if(!e||$.length===0)return;const t=$.reduce((r,n)=>r+n.timeMs,0);let o=`<div class="opt-summary">
  ...
  </div>`,e.innerHTML=o}function ne(){const e=E.querySelector("#graph-container");!e||$.length===0||(e.innerHTML=He()+Me())}function He(){const e=ee(),t=680,o=300,r=82,n=20,d=36,c=52,m=t-r-n,s=o-d-c,a=$.map(g=>g.totalEnergy),i=Math.min(...a),f=Math.max(...a),y=Math.max((f-i)*.15,.002),v=i-y,w=f+y,H=g=>d+s-(g-v)/(w-v)*s,u=$.length,p=m/F.length;let h=`<svg width="${t}" height="${o}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${t} ${o}" style="max-width:100%;">`;h+=`<rect x="${r}" y="${d}" width="${m}" height="${s}" fill="${e.surface}" rx="2"/>`;for(let g=0;g<=5;g++){const b=v+(w-v)*g/5,S=H(b);h+=`<line x1="${r}" y1="${S}" x2="${r+m}" y2="${S}" stroke="${e.grid}" stroke-width="0.5"/>`,h+=`<text x="${r-6}" y="${S+3}" text-anchor="end" font-size="9" fill="${e.dim}">${b.toFixed(4)}</text>`}const B=r+p*4;h+=`<line x1="${B}" y1="${d}" x2="${B}" y2="${d+s}" stroke="${e.grid}" stroke-width="1" stroke-dasharray="4,3"/>`;const l=r+p*2,k=r+p*6;h+=`<text x="${l}" y="${o-4}" text-anchor="middle" font-size="9" fill="${e.dim}" font-weight="600">Post-HF</text>`,h+=`<text x="${k}" y="${o-4}" text-anchor="middle" font-size="9" fill="${e.dim}" font-weight="600">DFT</text>`,h+=`<line x1="${r}" y1="${d}" x2="${r}" y2="${d+s}" stroke="${e.axis}" stroke-width="1"/>`,h+=`<line x1="${r}" y1="${d+s}" x2="${r+m}" y2="${d+s}" stroke="${e.axis}" stroke-width="1"/>`;const M=$[0].totalEnergy;for(let g=0;g<u;g++){const b=$[g],S=R(b.method.id),z=r+p*g+p/2,L=H(b.totalEnergy);if(b.method.type!=="hf"){const A=H(M),Z=r+p*g+p*.15,se=p*.7,ie=Math.min(L,A),de=Math.abs(A-L);h+=`<rect x="${Z}" y="${ie}" width="${se}" height="${de}" fill="${S}" opacity="0.25" rx="2"/>`}const U=r+p*g+p*.1,X=r+p*g+p*.9;if(h+=`<line x1="${U}" y1="${L}" x2="${X}" y2="${L}" stroke="${S}" stroke-width="3" stroke-linecap="round"/>`,h+=`<text x="${z}" y="${L-8}" text-anchor="middle" font-size="8" fill="${S}" font-weight="600">${b.totalEnergy.toFixed(4)}</text>`,h+=`<text x="${z}" y="${d+s+14}" text-anchor="middle" font-size="9" fill="${e.dim}" font-weight="600">${b.method.label}</text>`,b.method.type==="dft"&&(h+=`<text x="${z}" y="${d+s+25}" text-anchor="middle" font-size="7" fill="${e.dim}" opacity="0.7">${b.method.category}</text>`),g>0&&g!==4){const A=H($[g-1].totalEnergy),Z=r+p*(g-1)+p/2;h+=`<line x1="${Z}" y1="${A}" x2="${z}" y2="${L}" stroke="${e.grid}" stroke-width="1" stroke-dasharray="4,3"/>`}}return h+=`<text x="${r+m/2}" y="20" text-anchor="middle" font-size="12" font-weight="600" fill="${e.titleSvg}">${x("acc.graphTitle")}</text>`,h+=`<text x="14" y="${d+s/2}" text-anchor="middle" font-size="10" fill="${e.dim}" transform="rotate(-90,14,${d+s/2})">${x("acc.yEnergy")}</text>`,h+="</svg>",h}function Me(){const e=ee(),t=$.filter(l=>l.method.type!=="hf");if(t.length<1)return"";const o=680,r=200,n=82,d=20,c=28,m=48,s=o-n-d,a=r-c-m,i=t.length,f=s/i,y=t.map(l=>l.diffFromHF),w=Math.max(Math.abs(Math.min(...y)),Math.abs(Math.max(...y)),1)*1.2,H=l=>c+a/2-l/w*(a/2);let u=`<svg width="${o}" height="${r}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${o} ${r}" style="max-width:100%;margin-top:8px;">`;u+=`<rect x="${n}" y="${c}" width="${s}" height="${a}" fill="${e.surface}" rx="2"/>`;const p=t.filter(l=>l.method.type==="postHF").length;if(p>0&&p<i){const l=n+f*p;u+=`<line x1="${l}" y1="${c}" x2="${l}" y2="${c+a}" stroke="${e.grid}" stroke-width="1" stroke-dasharray="4,3"/>`}const h=4;for(let l=-h;l<=h;l++){const k=w*l/h,M=H(k);M<c||M>c+a||(u+=`<line x1="${n}" y1="${M}" x2="${n+s}" y2="${M}" stroke="${e.grid}" stroke-width="${l===0?1:.5}"/>`,u+=`<text x="${n-6}" y="${M+3}" text-anchor="end" font-size="8" fill="${e.dim}">${k.toFixed(0)}</text>`)}const B=H(0);u+=`<line x1="${n}" y1="${B}" x2="${n+s}" y2="${B}" stroke="${e.axis}" stroke-width="1"/>`,u+=`<line x1="${n}" y1="${c}" x2="${n}" y2="${c+a}" stroke="${e.axis}" stroke-width="1"/>`;for(let l=0;l<i;l++){const k=t[l],M=R(k.method.id),g=n+f*l+f/2,b=n+f*l+f*.2,S=f*.6,z=H(y[l]),L=Math.abs(z-B),U=Math.min(z,B);u+=`<rect x="${b}" y="${U}" width="${S}" height="${L}" fill="${M}" opacity="0.75" rx="2"/>`;const X=y[l]<0?z+12:z-5;u+=`<text x="${g}" y="${X}" text-anchor="middle" font-size="9" fill="${M}" font-weight="700">${y[l].toFixed(1)}</text>`,u+=`<text x="${g}" y="${c+a+14}" text-anchor="middle" font-size="9" fill="${e.dim}" font-weight="600">${k.method.label}</text>`,k.method.type==="dft"&&(u+=`<text x="${g}" y="${c+a+25}" text-anchor="middle" font-size="7" fill="${e.dim}" opacity="0.7">${k.method.category}</text>`)}if(p>0&&p<i){const l=n+f*p/2,k=n+f*(p+(i-p)/2);u+=`<text x="${l}" y="${r-4}" text-anchor="middle" font-size="9" fill="${e.dim}" font-weight="600">Post-HF</text>`,u+=`<text x="${k}" y="${r-4}" text-anchor="middle" font-size="9" fill="${e.dim}" font-weight="600">DFT</text>`}return u+=`<text x="${n+s/2}" y="16" text-anchor="middle" font-size="11" font-weight="600" fill="${e.titleSvg}">${x("acc.diffTitle")}</text>`,u+=`<text x="14" y="${c+a/2}" text-anchor="middle" font-size="9" fill="${e.dim}" transform="rotate(-90,14,${c+a/2})">${x("acc.yDiff")}</text>`,u+="</svg>",u}let Q=!1;function Ce(){if(Q)return;Q=!0;const e=document.createElement("style");e.textContent=`
