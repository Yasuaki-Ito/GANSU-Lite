import{i as j,c as P,r as A,t as d,a as U,b as D,g as V,d as X}from"./nav-BaYBNpRD.js";import{p as Y}from"./parseXYZ-DTmoiCYR.js";import{t as Z,u as _,d as J}from"./builder-B-MUn5dT.js";const G=[{id:"h2",labelKey:"conv.scenH2",descKey:"conv.descH2",charge:0,mult:1,xyz:`2
H2
H  0.0  0.0  0.0
...
`)})()}],y=["STO-3G","STO-6G","3-21G","6-31G","6-31G(d,p)","cc-pVDZ"];let k=G[0],g=[],u=!1,L=!1,$=!1,O=0;const q=new Map;async function Q(i){const e=q.get(i);if(e)return e;const r=`/Program%20Files/Git/GANSU-Lite/basis/${i.toLowerCase()}.gbs`,o=await fetch(r);if(!o.ok)throw new Error(`Failed to load basis set: ${i}`);const t=await o.text(),n=J.fromGBS(t);return q.set(i,n),n}const h=document.getElementById("app");function v(){h.innerHTML=`
    <div class="opt-page">
      ${A("convergence")}
    ...
    </div>`,et();const i=h.querySelector("#mol-grid"),e=document.createElement("div");e.className="opt-category-row";for(const r of G){const o=document.createElement("div");o.className="opt-scenario-card"+(r.id===k.id?" selected":""),o.innerHTML=`<strong>${d(r.labelKey)}</strong><span class="conv-desc">${d(r.descKey)}</span>`,o.addEventListener("click",()=>{u||(k=r,g=[],$=!1,v())}),e.appendChild(o)}i.appendChild(e),h.querySelector("#nav-theme").addEventListener("click",()=>{U(),v()}),h.querySelector("#nav-lang").addEventListener("click",()=>{D(),v()}),h.querySelector("#run-btn").addEventListener("click",()=>{u||W()}),h.querySelector("#stop-btn")?.addEventListener("click",()=>{L=!0}),g.length>0&&(I(),R())}async function W(){u=!0,L=!1,g=[],$=!1,v();const i=Math.floor((k.mult-1)/2),e=performance.now(),r=Y(k.xyz);for(let o=0;o<y.length&&!L;o++){const t=y[o];tt(t,o,y.length);try{const n=performance.now(),a=await Q(t),b=new Z(r,a,k.charge,i),p=await _(b,"RHF").solve({eriBackend:"js"}),x=performance.now()-n;g.push({basis:t,nbasis:b.numBasis,energy:p,timeMs:x}),I(),R()}catch(n){const a=n instanceof Error?n.message:String(n);console.error(`Error computing ${t}:`,n),g.push({basis:t,nbasis:0,energy:NaN,timeMs:0,error:a})}await new Promise(n=>setTimeout(n,0))}O=performance.now()-e,u=!1,$=!0,v()}function tt(i,e,r){const o=h.querySelector("#progress-area");if(!o)return;const t=(e/r*100).toFixed(0);o.innerHTML=`
    <div class="opt-progress"><div class="opt-progress-bar" style="width:${t}%"></div></div>
    <div class="opt-progress-text">${i}... (${e}/${r})</div>`}function R(){const i=h.querySelector("#result-table");if(!i||g.length===0)return;const e=g.filter(n=>isFinite(n.energy)),r=e.length>0?e[e.length-1]:null,o=$?` (${(O/1e3).toFixed(1)}s)`:"";let t=`<div class="opt-summary">
