import"./styles-C22XBgWP.js";import{p as A}from"./parseXYZ-BV3t15l8.js";import{g as j,b as I,B as K,f as G}from"./ri-Dh08ihv_.js";import{a as P,b as D}from"./theoryControls-BlyfxdhA.js";import{a as U,b as Y}from"./properties-mfV5hVSU.js";import{i as _,c as V,r as Q,t as i,a as W,b as X,g as Z}from"./nav-B8zugkj2.js";import{e as J}from"./radioGroup-C7LT8iiA.js";import{a as ee}from"./moleculeViewer3D-C4Xiya5A.js";const z=[{id:"h2",labelKey:"chg.scenH2",descKey:"chg.descH2",category:"homo",charge:0,mult:1,xyz:`2
H2
H  0.0  0.0  0.0
...
`)})()}],te={homo:"chg.catHomo",hetero:"chg.catHetero",poly:"chg.catPoly",ion:"chg.catIon"};let F=z[0],f=new Map,g=!1,L=!1,C="HF";const T=new Map;async function oe(t){const e=T.get(t);if(e)return e;const o=`/GANSU-Lite/basis/${t.toLowerCase()}.gbs`,r=await fetch(o);if(!r.ok)throw new Error(`Failed to load basis set: ${t}`);const n=await r.text(),a=G.fromGBS(n);return T.set(t,a),a}const m=document.getElementById("app");function v(){const t=f.has(F.id);m.innerHTML=`
    <div class="opt-page">
      ${Q("charges")}
    ...
    </div>`,le();const e=m.querySelector("#scen-grid"),o=["homo","hetero","poly","ion"];for(const n of o){const a=z.filter(x=>x.category===n);if(a.length===0)continue;const u=document.createElement("div");u.className="chg-cat-label",u.textContent=i(te[n]),e.appendChild(u);const s=document.createElement("div");s.className="opt-category-row";for(const x of a){const h=document.createElement("div"),d=f.has(x.id);h.className="opt-scenario-card"+(x.id===F.id?" selected":"")+(d?" computed":""),h.innerHTML=`<strong>${i(x.labelKey)}</strong><span class="conv-desc">${i(x.descKey)}</span>`,h.addEventListener("click",()=>{g||(F=x,v())}),s.appendChild(h)}e.appendChild(s)}J(e,{label:i("chg.molecule")}),m.querySelector("#nav-theme").addEventListener("click",()=>{W(),v()}),m.querySelector("#nav-lang").addEventListener("click",()=>{X(),v()});const r=m.querySelector("#theory-sel");r&&r.addEventListener("change",()=>{C=r.value}),m.querySelector("#run-btn").addEventListener("click",()=>{g||re(F)}),m.querySelector("#run-all-btn").addEventListener("click",()=>{g||ne()}),m.querySelector("#stop-btn")?.addEventListener("click",()=>{L=!0}),t&&(ie(f.get(F.id)),ce(f.get(F.id))),f.size>1&&se()}async function E(t){const e=await oe("STO-3G"),o=A(t.xyz),r=Math.floor((t.mult-1)/2),n=new j(o,e,t.charge,r),a=performance.now(),u=await D(n,e,C);await u.solve({eriBackend:"js"});const s=performance.now()-a,x=U(u.density,u.overlap,n.atoms,n.atomToBasisRange),h=Y(u.density,n.atoms,n.primitiveShells,n.cgtoNormalizationFactors,n.numBasis),d=[],b=[];for(const y of n.atoms)d.push(I(y.atomicNumber)),b.push({x:y.coordinate.x*K,y:y.coordinate.y*K,z:y.coordinate.z*K});return{molId:t.id,charges:Array.from(x),dipole:h,atomSymbols:d,atomCoords:b,timeMs:s}}async function re(t){g=!0,v();try{const e=await E(t);f.set(t.id,e)}catch(e){console.error("Charge computation error:",e)}g=!1,v()}async function ne(){g=!0,L=!1,v();for(let t=0;t<z.length&&!L;t++){const e=z[t];ae(t,z.length,i(e.labelKey));try{const o=await E(e);f.set(e.id,o)}catch(o){console.error(`Error computing ${e.id}:`,o)}await new Promise(o=>setTimeout(o,0))}g=!1,v()}function ae(t,e,o){const r=m.querySelector("#progress-area");if(!r)return;const n=(t/e*100).toFixed(0);r.innerHTML=`
    <div class="opt-progress"><div class="opt-progress-bar" style="width:${n}%"></div></div>
    <div class="opt-progress-text">${o} (${t+1}/${e})</div>`}function ce(t){const e=m.querySelector("#summary-area");if(!e)return;let o=`<div class="opt-summary">
    <h3>${i("chg.done")}</h3>
    <table>
      <tr>
        <th>${i("chg.colAtom")}</th>
        <th>${i("chg.colCharge")}</th>
      </tr>`;for(let r=0;r<t.atomSymbols.length;r++){const n=t.charges[r],a=R(n);o+=`<tr>
  ...
  </div>`,e.innerHTML=o}function R(t){const o=Math.max(-.6,Math.min(.6,t))/.6;if(o>0){const r=Math.round(255*(1-o)),n=Math.round(255*(1-o));return`rgb(${r},${n},255)`}else{const r=-o,n=Math.round(255*(1-r)),a=Math.round(255*(1-r));return`rgb(255,${n},${a})`}}function ie(t){const e=m.querySelector("#mol-vis");e&&ee(e,t)}function se(){const t=m.querySelector("#compare-chart");if(!t||f.size<2)return;const e=Z(),o=[];for(const l of z){const c=f.get(l.id);if(!c)continue;let $=0;for(let w=1;w<c.atomSymbols.length;w++)c.atomSymbols[w]!=="H"&&c.atomSymbols[$]==="H"&&($=w);const k=c.atomSymbols[$];o.push({label:`${i(l.labelKey)} (${k})`,charge:c.charges[$]})}if(o.length<2)return;const r=560,n=200,a=82,u=20,s=28,x=60,h=r-a-u,d=n-s-x,b=Math.max(.1,...o.map(l=>Math.abs(l.charge)))*1.2,y=h/o.length,S=l=>s+d/2-l/b*(d/2);let p=`<svg width="${r}" height="${n}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${r} ${n}" style="max-width:100%;margin-top:12px;">`;p+=`<rect x="${a}" y="${s}" width="${h}" height="${d}" fill="${e.surface}" rx="2"/>`;const H=S(0);p+=`<line x1="${a}" y1="${H}" x2="${a+h}" y2="${H}" stroke="${e.axis}" stroke-width="1"/>`;const q=[b*.5,-b*.5,b,-b];for(const l of q){const c=S(l);c>=s&&c<=s+d&&(p+=`<line x1="${a}" y1="${c}" x2="${a+h}" y2="${c}" stroke="${e.grid}" stroke-width="0.5"/>`,p+=`<text x="${a-6}" y="${c+3}" text-anchor="end" font-size="8" fill="${e.dim}">${l>=0?"+":""}${l.toFixed(2)}</text>`)}p+=`<text x="${a-6}" y="${H+3}" text-anchor="end" font-size="8" fill="${e.dim}">0</text>`,p+=`<line x1="${a}" y1="${s}" x2="${a}" y2="${s+d}" stroke="${e.axis}" stroke-width="1"/>`;for(let l=0;l<o.length;l++){const c=o[l],$=a+y*l+y/2,k=a+y*l+y*.2,w=y*.6,M=S(c.charge),N=R(c.charge);c.charge>=0?p+=`<rect x="${k}" y="${M}" width="${w}" height="${H-M}" fill="${N}" opacity="0.85" rx="2"/>`:p+=`<rect x="${k}" y="${H}" width="${w}" height="${M-H}" fill="${N}" opacity="0.85" rx="2"/>`;const O=c.charge>=0?M-5:M+12;p+=`<text x="${$}" y="${O}" text-anchor="middle" font-size="8" fill="${R(c.charge)}" font-weight="700">${c.charge>=0?"+":""}${c.charge.toFixed(3)}</text>`,p+=`<text x="${$}" y="${s+d+12}" text-anchor="middle" font-size="7.5" fill="${e.dim}" transform="rotate(-30,${$},${s+d+12})">${c.label}</text>`}p+=`<text x="${a+h/2}" y="16" text-anchor="middle" font-size="11" font-weight="600" fill="${e.titleSvg}">${i("chg.compareTitle")}</text>`,p+=`<text x="14" y="${s+d/2}" text-anchor="middle" font-size="9" fill="${e.dim}" transform="rotate(-90,14,${s+d/2})">${i("chg.yCharge")}</text>`,p+="</svg>",t.innerHTML=p}let B=!1;function le(){if(B)return;B=!0;const t=document.createElement("style");t.textContent=`
